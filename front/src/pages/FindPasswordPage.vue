  ...
  <UserCardLayout>
    ...
    <form v-if="step === 1" class="pw-form" @submit.prevent="onNext">
      <div class="form-group">
        <label>전화번호</label>
        <input v-model="phoneNum" class="input" placeholder="전화번호를 입력하세요" />
      </div>
      <div class="form-group">
        <label>이메일</label>
        <div class="input-row">
          <input v-model="email" class="input" placeholder="이메일을 입력하세요" />
          <button type="button" class="btn" @click="onVerifyUser">인증</button>
        </div>
        <div v-if="userVerified === false" class="error-msg">사용자의 정보와 일치하지 않습니다</div>
        <div v-if="userVerified === true" class="success-msg">사용자 정보 확인 완료</div>
      </div>
