...
public class BankCodeController {
    ...
    @GetMapping
    @Operation(summary = "은행 목록 조회", description = "활성화된 모든 은행 목록을 조회합니다.")
    public ResponseEntity<List<BankCodeDTO>> getAllBanks() {
        List<BankCodeDTO> banks = bankCodeService.getAllActiveBanks();
        return ResponseEntity.ok(banks);
    }
    @GetMapping("/{bankCode}")
    @Operation(summary = "은행 정보 조회", description = "특정 은행 코드의 정보를 조회합니다.")
    ...
    @PostMapping
    @Operation(summary = "은행 추가", description = "새로운 은행을 추가합니다.")
    ...
    @PutMapping("/{bankCode}")
    @Operation(summary = "은행 정보 수정", description = "기존 은행 정보를 수정합니다.")
    ...
    @Operation(summary = "은행 삭제", description = "은행을 삭제합니다.")
